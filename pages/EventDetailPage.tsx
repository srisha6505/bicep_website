      ...
      <div className="relative h-[50vh] bg-cover bg-center" style={{ backgroundImage: `url(${event.banner})` }}>
        <div className="absolute inset-0 bg-black bg-opacity-70 flex items-center justify-center">
          <div className="text-center text-white p-4">
            ...
            <p className="text-xl mt-2">{event.tagline}</p>
            <div className="mt-6 text-lg font-semibold space-x-6">
                    ...
                    <div className="space-y-4">
                        {event.agenda.map((item, index) => (
                            <div key={index} className="flex items-center bg-secondary p-4 rounded-lg">
                                <span className="font-bold text-primary w-28">{item.time}</span>
                                <p>{item.topic}</p>
                            </div>
                        ))}
                    </div>
            ...
            {event.speakers && event.speakers.length > 0 && (
             ...
             {event.status === 'Past' && (
                ...
                {event.report && <div className="bg-card border border-border p-8 rounded-xl shadow-sm">
                    <h3 className="text-2xl font-bold mb-4">Event Summary</h3>
                    <p className="text-muted-foreground bg-secondary p-4 rounded-lg">{event.report}</p>
                </div>}
          ...
          <div className="lg:col-span-1">
            <div className="bg-card border border-border p-6 rounded-xl shadow-md sticky top-24">
              ...
              {event.status === 'Upcoming' ? (
                <>
                  <p className="mb-4 text-green-400 font-bold text-lg">Status: Open</p>
