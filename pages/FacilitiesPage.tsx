  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <div className="text-center mb-12">
        <h1 className="text-4xl font-extrabold">Our Facilities</h1>
      ...
      <div className="space-y-16">
        {facilities.map((facility, index) => (
          <div key={facility.id} className={`flex flex-col ${index % 2 === 0 ? 'lg:flex-row' : 'lg:flex-row-reverse'} gap-8 items-center`}>
            ...
            <div className="lg:w-1/2">
              <span className="text-primary font-semibold">{facility.type}</span>
              <h2 className="text-3xl font-bold mt-2 mb-4">{facility.name}</h2>
              ...
              <div className="flex flex-wrap gap-4">
                {facility.equipment && (
                 ...
                 {facility.capacity && (
                    <div>
                        <h4 className="font-semibold">Capacity: <span className="text-muted-foreground">{facility.capacity} people</span></h4>
                    </div>
                 )}
              </div>
               <button className="mt-8 bg-primary text-primary-foreground font-semibold px-6 py-2 rounded-lg hover:bg-primary/90 transition-colors">
                    Request Booking
                </button>
            </div>
          </div>
